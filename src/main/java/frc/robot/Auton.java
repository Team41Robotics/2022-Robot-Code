...
public class Auton {
    ...
    private static long autonHumanStationWait, autonShootingStartTime, startTime;
    ...
    * @return Either current or next state, depending on readiness
    */
    public static AutonState gotoBallNoVision() {
        Shooter.setSpeed(Constants.SHOOTER_DEFAULT_SPEED);
        Hood.home();
        Limelight.setLedOn(true);
        // Beam break here
        ...
        Drivetrain.runInverseKinematics(0, Constants.AUTON_SPEED_M_PER_S * (2.0 / 3));
        return AutonState.GOTO_BALL;
    }
    ...
    */
    ...
    public static AutonState moveForward1Ball() {
        Shooter.setSpeed(Constants.SHOOTER_DEFAULT_SPEED);
        Hood.home();
        Limelight.setLedOn(true);
        if (Drivetrain.getPosition() > 100) {
            do {
                Drivetrain.setPosition(0);
            } while (Drivetrain.getPosition() > 1);
            return AutonState.TRACK_GOAL;
    ...
    public static void delayed1Ball() {
        autonShootingStartTime = System.currentTimeMillis();
        Limelight.setLedOn(true);
        Intake.runConveyor(true);
        if (Drivetrain.getPosition() < 100) {
            Drivetrain.runInverseKinematics(0, 0.75);
            double speed = (110 * Constants.HOOD_SPEED_SLOPE) + Constants.HOOD_SPEED_OFFSET + 0.5;
            double angle = (110 * 110 * Constants.HOOD_ANGLE_CURVE) + (110 * Constants.HOOD_ANGLE_SLOPE);
            Shooter.setSpeed(speed);
            Hood.setToPosition(angle);
        } else if (!Hood.isHomed()) {
            Hood.home();
            Drivetrain.runInverseKinematics(0, 0);
            // setupShoot();
            startTime = System.currentTimeMillis();
            ...
            // setupShoot();
            if (Hood.isReady() && Shooter.isReady()){// && Drivetrain.alignToGoal()) {
    ...
    */
    private static void setupShoot() {
        ...
        double speed = (distance * Constants.HOOD_SPEED_SLOPE) + Constants.HOOD_SPEED_OFFSET + 0.5;
